...
import {Alert, FlatList, TouchableOpacity, ScrollView, Button, Platform, Image, ImageBackground, Dimensions, StyleSheet, Text, TextInput, TouchableWithoutFeedback, View} from "react-native";
...
let self = this;
export default class CarefreeBBSIos extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            slideshowArr:'',
    ...
    renderTopicItem = ({item}) => {
        return (
            <View style={styles.topicItem}>
                <Image style={styles.topicImg} source={{uri:global.PicUrl + item.img}}/>
            ...
            <View key={index} style={{borderBottomWidth: 1, borderBottomColor: "#DDDDDD"}}>
                <View style={styles.txncqy}>
                    <View style={{flexDirection:"row",alignItems:"center"}}>
                        {
                            item.imgList ? <Image style={styles.bbsLtImgu} source={{uri:global.PicUrl+item.imgList[0]}}/> : <Image style={styles.bbsLtImgu} source={require('./Images/rumen.png')}/>
                        }
                    ...
                    <View style={styles.txncqy3}>
                        {
                            ...
                            <View style={styles.txncqy31}>
                            <Image style={{width:15,height:16}} source={require('./Images/dianzan.png')}/>
                            <Text style={styles.txncqy31Text}>点赞</Text>
                            </View>
                            </TouchableWithoutFeedback>
                            }
        ...
        return (
            <View style={styles.containerAppCare}>
                <View style={{width:width,flex:1}}>
                    {/*太极团*/}
                    {/*{*/}
                    {/*    this.state.groupList.length > 0 ? <View style={{height:height * 0.3}}>*/}
                ...
                {/*评论输入框*/}
                {
                    this.state.inputState ? <View style={styles.box}>
                        <TextInput onChangeText={(text) => this.setState({comment: text})} onBlur={() => this.setState({inputState: false})} autoFocus={this.state.inputState} ref="input"
                                   style={styles.input} placeholderTextColor='#999999' placeholder={'评论内容'} underlineColorAndroid="transparent"/>
