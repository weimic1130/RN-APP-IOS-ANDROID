...
export default class OrderPayment2Android extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            purchaseNum: 0,
                        ...
                        {/*顶部*/}
                        {
                            this.state.addressText ?
                                <View style={styles.modalSty4}>
                                    <Text style={styles.modalSty4Text1}>{this.state.addressText.name} {this.state.addressText.tel}</Text>
                                    <Text style={styles.modalSty4Text2}>{this.state.addressText.address}</Text>
                                </View> : null
                        }
                        {/*支付方式*/}
                        <View style={styles.modalSty2}>
                            ...
                            <View style={styles.modalSty5}>
                                <View style={styles.modalSty5Child}>
                                    ...
                                    <Text style={styles.modalSty4Text3}>{this.state.purchaseNum}</Text>
                                </View>
                                <View style={styles.modalSty5Child}>
                                    <Text style={styles.modalSty4Text3}>支付金额：</Text>
                                    <Text>￥{this.state.NpayOrder.totalMoney ? (this.state.NpayOrder.totalMoney / 100).toFixed(2) : 0}</Text>
                                </View>
                                <View style={styles.modalSty5Child}>
                                    <Text style={styles.modalSty4Text3}>总运费：</Text>
                                    <Text>{this.state.NpayOrder.logisticsMoney == 0 ? '免运费' : this.state.NpayOrder.logisticsMoney / 100}</Text>
                                </View>
                            </View>
                            {
                ...
                <View style={styles.OrderBtm}>
                    <View style={styles.shurukacon3}>
                        <Text style={styles.shurukacon3BtmText}>总计：
                            <Text style={styles.shurukacon3BtmText2}>{(this.state.NpayOrder.totalMoney / 100).toFixed(2)}</Text></Text>
                    ...
                    {
                        this.state.payDetail ? <View>
                            <View style={styles.shurukacon7}>
                                <View style={styles.shurukacon71}>
                                    <Text style={styles.shurukacon711}>12额2的</Text>
                                    <Text style={styles.shurukacon712}>￥120.00</Text>
                                </View>
                                <View style={styles.shurukacon71}>
                                    <View style={styles.shurukacon73}>
                                        <Text style={styles.shurukacon73Text}>商品运费</Text>
                                    </View>
                                    <Text style={styles.shurukacon74Text}>￥5.00</Text>
                                </View>
                                <View style={styles.shurukacon71}>
                                    <View style={styles.shurukacon714}>
                                        <Text style={styles.shurukacon7142}>会员折扣</Text>
                                    </View>
                                    <Text style={styles.shurukacon71Text}>-￥50.00</Text>
                                </View>
                            </View>
                            <View style={styles.shurukacon8}>
                                <Text style={styles.shurukacon8Text1}>总计：<Text style={styles.shurukacon8Text2}>￥612.00</Text></Text>
                            </View>
                        </View> : null
                    }
                </View>
    ...
    confirmPayment = (parameter) => {
        let self = this;
        console.log(JSON.parse(this.state.NpayOrder.goodsInfo));
        let logisticsMoney = 0;
        let dataList = JSON.parse(this.state.NpayOrder.goodsInfo);
        for (let i = 0; i < dataList.length; i++) {
            logisticsMoney = logisticsMoney + dataList[i].logisticsMoney;
        }
        ...
        formData.append('logisticsMoney', logisticsMoney);
        let opts = {
            method: 'POST',
            body: formData
        };
            ...
            .then((res) => {
                ...
                if (res.code == 0) {
                    this.setState({
                        ...
                        NpayOrder: res.data,
                        goodsList: JSON.parse(res.data.goodsInfo)
                    })
                    let num = 0;
                    for (let i = 0; i < this.state.goodsList.length; i++) {
                        num = num + this.state.goodsList[i].num;
                    }
                    this.setState({
                        purchaseNum: num
                    })
    ...
    componentDidMount() {
        ...
        if (this.props.navigation.state.params.batchId) {
            this.getPayOrder(this.props.navigation.state.params.batchId);
        }
    }
